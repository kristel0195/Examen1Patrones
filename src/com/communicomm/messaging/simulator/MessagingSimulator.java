...
import com.communicomm.messaging.messages.controller.SendMessage;
...
public class MessagingSimulator {
	...
	private static void processOption(int option) throws Exception  {
		...
		if (option == 1){
			createMessage(Messages.values()[messageOption -1]);	
		}
		if (option == 2){
	...
	private static Message createMessage(Messages type) throws Exception {
		return SendMessage.send(type);
	}
